...
const CreateRent = (async (req, res) => {
    const { id, remark, roomNumber, billMonth, start, end, unit, roomRent, total, prevAdvance,
    ...
    try {
        ...
        if (!id) {
            ...
            newRent.save().then(() => {
             ...
             await RentModal.findByIdAndUpdate(id, {
                paid: paid,
                paidOn: paidOn,
                remark: remark
            }, (err, docs) => {
                if (err) {
                    ...
                    res.status(200).json({ msg: "Rent Updated sucessfully" });
                }
            })
